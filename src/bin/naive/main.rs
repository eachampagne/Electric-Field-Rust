...
fn main() {
    ...
    //A few constants
    let TAU:f32 = 2.0 * PI;
    let EPSILON_NAUGHT:f32 = 8.854187817*10.0_f32.powf(-12.0); //F/m

    let n:u32 = 1000; //Number of divisions
    let R:f32 = 1.0; //arbitrary length
    let Q:f32 = 1.0; //arbitrary charge

    //Bounds of integration
    let theta1:f32 = 1.0 * PI / 180.0;
    let theta2:f32 = PI;
    let phi1 = 0.0;
    ...
    let dtheta = (theta2 - theta1) / n as f32;
    let dphi = (phi2 - phi1) / n as f32;
    ...
    let mut A:f32 = 0.0;
    for j in 0..n { //theta
        let theta:f32 = theta1 + (j as f32 + 0.5) * dtheta;
        for k in 0..n { //phi
            let phi:f32 = phi1 + (k as f32 + 0.5) * dphi;
            A += (theta.sin() * dtheta * dphi * R.powf(2.0));
        }
    }

    let SIGMA:f32 = Q/A; //Surface charge density
    ...
    for i in 0..501 { //where along z-axis we're measuring
        let z:f32 = i as f32 / 100.0;
        let mut integralSum = Vector::new(0.0, 0.0, 0.0);
        let testPoint = Vector::new(0.0, 0.0, z);
        for j in 0..n {//theta
            let theta = theta1 + (j as f32 + 0.5) * dtheta;
            for k in 0..n { //phi
                let phi = phi1 + (k as f32 + 0.5) * dphi;
                let currentVector:SphereVector = SphereVector::new(R, theta, phi); //Coordinates of point on shell
                let shellVector:Vector = currentVector.toCartesian() - testPoint; //from z-axis to shell
                ...
                currentField = toShell.normalize() * (1.0 * theta.sin() * dtheta * dphi * toShell.mag().powf(-2.0)); //Commentary: changed to mag rather than r because my fields are not public
